...
CREATE TABLE SACE.CONTA_BANCO (
       codContaBanco VARCHAR(10) NOT NULL
     , nomeConta CHAR(10)
     , valor DECIMAL(10, 2)
     , PRIMARY KEY (codContaBanco)
);

CREATE TABLE SACE.TIPO_MOVIMENTACAO_CONTA (
...
CREATE TABLE SACE.PLANO_CONTA (
       codPlanoConta BIGINT NOT NULL AUTO_INCREMENT
     , descricao VARCHAR(40) NOT NULL
     , codContaPai INT
     , tipoConta CHAR(1)
     , diaBase SMALLINT
     , codContaBanco VARCHAR(10)
     , PRIMARY KEY (codPlanoConta)
     , INDEX (codContaBanco)
     , CONSTRAINT FK_PLANO_CONTA_1 FOREIGN KEY (codContaBanco)
                  REFERENCES SACE.CONTA_BANCO (codContaBanco)
);
...
CREATE TABLE SACE.CONTA_PAGAR (
     ...
     , CONSTRAINT FK_CONTA_PAGAR_1 FOREIGN KEY (codPlanoConta)
                  REFERENCES SACE.PLANO_CONTA (codPlanoConta)
     , INDEX (codPessoa)
...
CREATE TABLE SACE.TB_ENTRADA_PAGAMENTO (
     ...
     , CONSTRAINT FK_TB_ENTRADA_PAGAMENTO_3 FOREIGN KEY (codPagamento)
                  REFERENCES SACE.CONTA_PAGAR (codPagamento)
);
...
CREATE TABLE SACE.CONTA_RECEBER (
...
CREATE TABLE SACE.MOVIMENTACAO_CONTA (
     ...
     , CONSTRAINT FK_MOVIMENTACAO_CONTA_1 FOREIGN KEY (codTipoMovimentacao)
                  REFERENCES SACE.TIPO_MOVIMENTACAO_CONTA (codTipoMovimentacao)
     , INDEX (codContaBanco)
     , CONSTRAINT FK_MOVIMENTACAO_CONTA_2 FOREIGN KEY (codContaBanco)
                  REFERENCES SACE.CONTA_BANCO (codContaBanco)
